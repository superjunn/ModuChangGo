...
## 기대효과(발전가능성) 및 한계점
활용성(현재프로젝트에 구현되어 있으며 실제 군에서 사용할 시 활용 가능한 사항들)
    1. 자동화로 인한 편리성 😇
    	- 창고 재고의 입/출고가 RFID와 아두이노를 이용해 자동으로 진행됨.
	- 기존에 일일히 수기로 작성하해야 했던 기능들이 자동화되어 더욱 편리하게 창고 관리를 진행할 수 있음.
	- 수동으로 진행되던 일을 자동으로 진행함으로써 
